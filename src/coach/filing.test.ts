...
describe("inbound link counting", () => {
  ...
  it("does not attribute bare links when the filename is shared", () => {
    // README is the most common filename there is. Counting every [[README]]
    // link against one of them reported dozens of breakages that were not real.
    write("Raw phone/notes.md", "---\ntags: [daily]\n---\n\n## Entry\n\nx\n");
    write("Life/Moments/README.md", "# Moments\n");
    write("Life/Journal/2026-08-29.md", "[[README]] and [[README]]\n");
    write("Raw phone/README.md", "# inbox\n");
    const found = findMisfiled(config).find((m) => m.path === "Raw phone/notes.md");
    expect(found!.inboundLinks).toBe(0);
  });

  it("flags a shared filename so the count is not read as complete", () => {
    write("Mystery.md", "---\ntags: [x]\n---\n\ntext\n");
    write("Life/Moments/Mystery.md", "---\ntags: [moment]\n---\n\ntext\n");
    const found = findMisfiled(config).find((m) => m.path === "Mystery.md");
